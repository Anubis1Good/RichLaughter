import copy
from Traders.TestingTrader.wss_maps_without_MTA import bitgetFutMap,moexFutMap,moexStockMap,moexFutMap2,moexStockMap2
...
def add_skynet(suffix,tradeMap,hourss):
    ...
    for g in tradeMap:
        ...
        newMap[g].append((MTA_SKYNET,(100,filename)))
        filename = f"BF24_{g}_{suffix}"
        newMap[g].append((MTA_SKYNET,(100,filename)))
        filename = f"BF100_{g}_{suffix}"
        newMap[g].append((MTA_SKYNET,(100,filename)))
        filename = f"BTDF_{g}_{suffix}"
        newMap[g].append((MTA_SKYNET,(100,filename)))
        filename = f"LAT_{g}_{suffix}"
        newMap[g].append((MTA_SKYNET,(100,filename)))
        filename = f"PAT_{g}_{suffix}"
        newMap[g].append((MTA_SKYNET,(100,filename)))
        filename = f"SAT_{g}_{suffix}"
        newMap[g].append((MTA_SKYNET,(100,filename)))
