...
df = dynamic_zigzag_picks(df,method='mean',n_std=3)

points = df[~pd.isna(df['zigzag_peaks'])].iloc[:-1]
p1 = points.iloc[::2]
p2 = points.iloc[1::2]

df['delta_zz'] = p1['middle'].diff()
df['delta_zz'] = p2['middle'].diff()
df['delta_zz'] = df['delta_zz'].ffill()
df['line'] = df['middle'] + df['delta_zz']
print(df.head())
