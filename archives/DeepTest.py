...
def get_moment_df(row:pd.Series,slice_fast:pd.DataFrame,multiplier:int=1):
    ...
    x = slice_fast.iloc[-1]['x']
    last_bar = last_bar.copy()
    last_bar['x'] = x
    slice_index += 1
...
try:
    for i in range(period*2,len_df):
        ...
        while ret:
            ...
            print(len(slice_fast_copy.index))
            plt.plot(slice_fast_copy['open'])
            plt.plot(slice_fast_copy['close'])
            plt.pause(0.1)
            plt.clf()
            slice_fast_copy = ws.preprocessing(slice_fast_copy)
            # print(slice_fast_copy.tail())
            action = ws(slice_fast_copy.iloc[-1])
            # print(action, slice_fast_copy.iloc[-1])
